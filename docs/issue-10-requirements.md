...
| 3.2 | `.dockerignore` and hardened Compose with operator-owned persistence, env file, restart, health, grace period, and localhost-only health port                                      | `.dockerignore` plus `compose.yaml` provide a read-only root, dropped capabilities, an explicit validated host bind with `create_host_path: false`, required env file, restart/grace/health settings, and `127.0.0.1` binding.                                                                                   |
...
- 3.A Fresh-clone/one-command behavior and persistent state are covered by
  Compose rendering, exact-bind preflight, container health, and host-directory
  recovery tests.
